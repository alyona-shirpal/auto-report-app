...
export class ChatGptAiService {
  ...
  private currentCommits: CommitResponse[];
  ...
  private createCommitPrompt(): string  {
    const prompt = `Create a human readable response for tracking system of my daily work. The response information should include data from the array 
     ${JSON.stringify(this.currentCommits)}  - message, branch and repo. Generate response based on the information of commit message. Do not specify the time and date. Avoid any explanation `
     return prompt;
 }
